/*! \file    fractals_random.c
    ...
    Algorithm:
        ...
        - Define the affine transformations
          (of the form \f$r_{i+1} = A r_i + b \f$)
        - Find the stationary point for each transformation
        - To draw:
           - If you are at the lowest level, draw lines connecting all the
             stationary points
           - If not, call the draw function recursively with each affine
    ...
    \image   html openglut_fractals.png OpenGLUT Fractal Demonstration
    \include demos/Fractals_random/fractals_random.c
*/
...
main(int argc, char *argv[])
{
    ...
    /*Gtk set locale to good value, but ... czech is not ideal for reading files with float numbers*/
