/*! \file    CallbackMaker.c
    \ingroup demos

    A program for tracing the invocation of GtkGLUT callbacks
    in response to system events.

    \author  Portions copyright (C) 2004, the OpenGLUT project contributors. <br>
             OpenGLUT branched from freeglut in February, 2004.
 
    \image   html openglut_callbackmaker.png CallbackMaker
    \include CallbackMaker.c
*/
...
int joystick_a = -1, joystick_b = -1,
    joystick_c = -1, joystick_d = -1,
    joystick_seq = -1;  /* Need meaningful names */
...
static void Joystick( unsigned int a, int b, int c, int d)  /* Need meaningful names */
{
    int window = glutGetWindow( );
...
int main( int argc, char *argv[ ] )
{
    ...
    printf( "Back from the OpenGLUT main loop\n" );
    return EXIT_SUCCESS;
}
